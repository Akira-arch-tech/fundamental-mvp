...
定制页已升级到 v1：`/customize/[productId]` 支持上传图片、文本配色、预览、DPI 提示和 `POST /api/customizations` 保存；并可跳转 `/checkout` 完成下单（`POST /api/orders`），最终进入 `/orders/[id]` 查看履约时间线、工单/物流节点，以及客服入口与异常流程（改图/补发/退款）。
