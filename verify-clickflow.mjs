...
const checks = [
  ...
  { path: "/favorite", mustInclude: "オリジナル推し活グッズ" },
  { path: "/products", mustInclude: "商品一覧" },
  { path: "/products/free-acrylic-stand-clear", mustInclude: "デザインを始める" },
  { path: "/customize/p1", mustInclude: "デザインエディタ" },
  { path: "/checkout", mustInclude: "チェックアウト" },
];
...
async function runChecks() {
  ...
  const detailRes = await fetch(`${base}/orders/${orderId}`);
  if (!detailRes.ok) {
    throw new Error(`/orders/${orderId} returned ${detailRes.status}`);
  }
  const detailHtml = await detailRes.text();
  if (!detailHtml.includes("ご注文の進捗")) {
    throw new Error(`/orders/${orderId} missing marker: ご注文の進捗`);
  }
  if (!detailHtml.includes("お問い合わせ") || !detailHtml.includes("特別対応のお申し出")) {
    throw new Error(`/orders/${orderId} missing customer inquiry or exception panel (JP)`);
  }
  ...
  console.log(`PASS /orders/${orderId}`);
}
